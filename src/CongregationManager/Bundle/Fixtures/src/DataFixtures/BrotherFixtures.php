    ...
    public function load(ObjectManager $manager): void
    {
        foreach ($this->brotherFixtureData as $brotherData) {
            $brother = new Brother(
                ...
                $brotherData['first_name'],
                $this->getReference('congregation-' . $brotherData['congregation_name']),
                $brotherData['is_male'],
                $brotherData['middle_name'],
                $brotherData['birth_date'] ? new \DateTimeImmutable($brotherData['birth_date']) : null,
                $brotherData['baptism_date'] ? new \DateTimeImmutable($brotherData['baptism_date']) : null,
            );
            if (array_key_exists('user', $brotherData)) {
                ...
                $encodedPassword = $this->userPasswordHasher->hashPasswordForUser($brotherData['user']['password'], $user);
